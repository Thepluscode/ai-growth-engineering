"""Evidence-backed commercial signal intelligence for internal GrowthOps use.

This module deliberately stops before external execution. It turns observed commercial
signals into eligible, ranked prospect recommendations while preserving provenance and
unknowns. Eligibility is non-compensatory: a strong intent score cannot outvote a hard
disqualifier or missing identity/evidence.
"""
...
class IntentSignal:
    ...
    def validate(self) -> None:
        for field_name in (
            "signal_id", "prospect_id", "company", "signal_type", "source",
...
def freshness_weight(observed_at: str, *, today: date | None = None, half_life_days: int = 21) -> float:
...
def priority_score(value: PriorityInput, *, today: date | None = None) -> PriorityResult:
    ...
    Inputs are bounded and explainable. This is a priority score, never an eligibility
    score; callers must not use it to rescue an ineligible prospect.
    """
...
def signal_record(signal: IntentSignal) -> dict:
    """Convert a validated signal into the durable generic-registry representation."""
    signal.validate()
    return {
...
def recommendation_object(
    *,
    ...
    """The decision object GrowthOps surfaces to a human operator."""
